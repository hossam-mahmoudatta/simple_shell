...
int write_history(info_t *info)
{
	...
	{
		_putsFD(node->str, fileDelimiter);
		_putFD('\n', fileDelimiter);
	}
	_putFD(BUF_FLUSH, fileDelimiter);
	close(fileDelimiter);
	return (1);
}
...
int read_history(info_t *info)
{
	...
	ssize_t fileDelimiter, rdlen, fsize = 0;
	struct stat st;
	...
	fileDelimiter = open(filename, O_RDONLY);
	free(filename);
	if (fileDelimiter == -1)
		return (0);
	if (!fstat(fileDelimiter, &st))
		fsize = st.st_size;
	if (fsize < 2)
		return (0);
